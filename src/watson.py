...
class watson(baseAssistant):
    ...
    def __init__(self):
        ...
        self.assistant = AssistantV1(
            version="2018-07-10", authenticator=self.authenticator
        )
    ...
    def getResponse(self, msg="bye"):
        response = self.assistant.message(
            workspace_id=self.creds["watson"]["WORKSPACE_ID"], input={"text": msg}
        ).get_result()
    ...
    def deleteIntent(self, intent):
        self.assistant.delete_intent(
            workspace_id=self.creds["watson"]["WORKSPACE_ID"], intent=intent["intent"]
        ).get_result()
