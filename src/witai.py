...
class wit(baseAssistant):
    def __init__(self):
        self.LIMIT = 200
        self.creds = self.getCreds()
        self.URL = self.creds["wit"]["URL"]
        self.headers = {
            'Authorization': 'Bearer ' + self.creds["wit"]["ACCESS_TOKEN"],
    ...
    def _req(self, method, path, params=None):
        ...
        if rsp.status_code > 200:
            raise WitError(f'Wit status: {str(rsp.status_code)} -- {rsp.reason}')
        json = rsp.json()
        if 'error' in json:
    ...
    def getWorkspaceID(self):
        return self._req('GET', f'/apps/{self.creds["wit"]["APP_ID"]}')
    ...
    def deleteIntent(self, intent):
        return self._req("DELETE", f'/entities/{intent}')

    def getIntents(self):
        intents = self._req('GET', f'/entities')
        return [intent for intent in intents if "wit$" not in intent]

    def getResponse(self, msg):
        response = self._req('GET', f'/message', {"q": msg})
...
client = Wit('2MQMZOYRSSNKYID34PNZ4PI4X2IJKBVV')
client.message('set an alarm tomorrow at 7am')
client.get_samples(5)
client.get_apps(0)
client.get_app("3632875376737533")
